---
title: Analyzing Airbnb Property Data from Texas
...
The concept of having a theory session in the morning and a coding session in the afternoon was the perfect setup to run this course. The applications in R were presented in an accessible way and helped to understand the material covered in the theory blocks. Setting up a model can already be challenging in the first step, when it only exists on a piece of paper. However, it certainly becomes even more complex if you have to implement it in a statistical program such as R. The coding sessions took some of this complexity away and this is not only helpful with regard to this course, but will be highly beneficial in the research phase of my PhD. The two assignment were a nice way to apply the learned material for myself, to really check whether the concepts have been well understood. 
...
My five main takeaways are: 

1. beautiful data visualizations with maps
2. maximum likelihood estimation in R (full and concentrated)
3. creating weight matrices (k-nearest and distance-based)
4. computationally expensive operations with log determinants of large matrices
5. SAR, SDM and MESS models for spatial regression analysis

</div>

I did my best to demonstrate as much as possible from these main takeaways in the term paper. Unfortunately, the data that I analyzed as a term paper project did not show as much spatial dependence as I hoped and time constraints do not allow me find yet another data set. Nevertheless, it was an interesting topic and led to some intriguing insights and visualizations. Another possible exercise would have been to compare two properties with the same number of bedrooms in two different locations (e.g., Irving and Corpus Christi), but since we have seen that already during the course, I did not include it in this final version of the term paper. Thank you very much for offering this course, I will highly recommend it to other students. 
